...
pub_path_int = rospy.Publisher('Planning_Output1',Int32MultiArray, queue_size=10)
time.sleep(5)
#------------------------------------------------------------------------------#
## the node function
class node:
...
def update_all_then_map():
	...
	rospy.Subscriber('robot2' ,Int32MultiArray,callback3)

	# Subscribe to robot2 position from the over head cam
	rospy.Subscriber('robot3' ,Int32MultiArray,callback5)

	# Subscribe to robot3 position from the over head cam
	rospy.Subscriber('robot4' ,Int32MultiArray,callback7)
...
def callback2(data):# for goal of the robot to move
	global xB,yB
	xB=data.data[0]#in px #//17.5)#in cm
	#xB = int((round(xB))
