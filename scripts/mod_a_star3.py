...
pub_path_int = rospy.Publisher('Planning_Output3',Int32MultiArray, queue_size=10)
time.sleep(5)
#------------------------------------------------------------------------------#
## the node function
class node:
...
def update_all_then_map():
	...
	rospy.Subscriber('robot3' ,Int32MultiArray,callback1)
...
def callback2(data):# for goal of the robot to move
	global xB,yB
	xB=data.data[4]#in px #//17.5)#in cm
	#xB = int((round(xB))
