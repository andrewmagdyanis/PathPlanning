...
pub_path_int = rospy.Publisher('Planning_Output2',Int32MultiArray, queue_size=10)
time.sleep(5)
#------------------------------------------------------------------------------#
## the node function
class node:
...
def update_all_then_map():
	...
	rospy.Subscriber('robot2' ,Int32MultiArray,callback1)
...
def callback2(data):# for goal of the robot to move
	global xB,yB
	xB=data.data[2]#in px #//17.5)#in cm
	#xB = int((round(xB))
